...
从 v1.18.0 开始，现有 `dispatch provider start --provider host-codex` 是非阻塞、worktree-isolated 的 legacy Host Codex SDK 入口：它在短事务中登记 provider session 和 agent session，事务外为每个 assignment 创建 `.ai-team/runtime/worktrees/<run>/<task>/<agent>` 独立 git worktree，再启动后台 worker。该 provider 需要可选 extra `kafa[host-codex]`，其中包含 `openai-codex>=0.1.0b3`；base 安装不携带 SDK。它不能继承当前原生任务的完整权限模型，因此不是新任务的默认编排路径，并且默认 fail-closed；只有显式设置 `HARNESS_CODEX_LEGACY_HOST_POLICY=isolated-deny-all` 才接受固定的 `Sandbox.workspace_write + ApprovalMode.deny_all` 兼容边界。兼容性的 Spark policy 还必须显式提供 `HARNESS_CODEX_SPARK_MODEL`。最终 JSON 仍只作为 raw provider report 导入；交付资格仍必须由 `dispatch verify-attempt` 在 controller 侧重新执行目标命令后生成可信 evidence。
