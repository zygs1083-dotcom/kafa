...
当前发布版本是 **v1.8.0-beta.1**，架构代际定位为 **Codex Harness Kernel v4.1.0**。它只负责交付经过验证的代码和证据，不负责生产部署、上线发布、基础设施开通、生产迁移、密钥变更或付费资源创建。
...
| Agent E2E 评测 | `run_agent_e2e_eval.py` 用确定性 fixture 验证调度、provider raw report、controller verify、file claim、integration 阻断闭环 |
...
它会在 push 和 pull request 上运行结构校验、JSON 校验、Python 编译、运行时回归测试、runtime smoke、forward wrapper、本地 skill eval fixture、Agent E2E fixture 和 Kernel 诊断烟测。

## 版本状态

当前 README 描述的是 v1.8 beta / Kernel v4.1 插件格式：
