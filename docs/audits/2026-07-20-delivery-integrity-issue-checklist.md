...
- 变更边界：P0 production/runtime/schema 修复已完成；没有 commit、push、merge、tag、release、deploy、用户安装替换或业务项目数据库迁移
...
single/parallel 报告替代；clean committed HEAD、远端 license detection、远端
attestation 和 publish 仍因没有 commit/push/release 授权而明确为 `not-run`，不被
伪装成本地通过。
...
| KAFA-P2-1 | P2 | committed live Codex evidence 不是当前 HEAD | 当前候选证据已刷新；clean committed HEAD not-run | stale evidence |
...
whole-change discovery supersedes the earlier checkpoint: 776 total, 762
...
the 770-case targeted union and 776-case discovery both completed with zero
failure/error; the available local container capability executed rather than
being promoted from an older skip. Final independent bounded review reported
zero open Critical/High/Medium finding.

## P2：治理与采用层

### KAFA-P2-1：刷新并正确标记 live Codex evidence

**关闭状态：当前候选证据已刷新；clean committed HEAD 仍为 not-run。**

single 与 parallel 报告现在都绑定最终 executable candidate 的
`git_head=e3d46d9...`、
`status_sha256=41fcfbd12e5c6b02fc6a541599d7f4bfcb981f6e112ca7616ebad49a3bcb5e98`、
`workspace_sha256=f2f952f78008f24e4f9cf5c05d9a2c38ba258d41b8158ec2730f9d3965ddcb2e`
和同一 Codex binary digest。两者分别通过 1/1，persistent consistency
errors 均为空，并进入最终 770-case targeted union 与 776-case discovery。
工作树仍因本变更未获 commit 授权而是 dirty；因此这些报告是
exact-current-candidate capability evidence，不是 clean committed release
evidence。

**验收标准**

- [x] P0/P1 稳定后，在 exact current candidate 重跑 single 和 parallel real Native Codex profiles；
- [ ] clean committed reviewed HEAD release evidence：因没有 commit 授权而 `not-run`；
...
- [ ] GitHub 远端 license detection：当前 API 仍为 `null`，需获准 commit/push 后才能刷新，记为 `not-run`；
...
- [ ] 远端 GitHub attestation 产物：没有 push/release 授权，记为 `not-run`；
...
最终持久制品位于 `/private/tmp/kafa-final-dist-f2f952f7/`。Wheel 为
46,640 B、SHA-256
`9f22bd8de0ea33ca76a4cf18f91a2e3f339cf9f496f1b14703f816d57b3bac6b`；
sdist 为 498,645 B、SHA-256
`444ee1c3a6328289ce0d576880a84ca284081d08fc62be3578b2c28418bbbddf`。
...
最终固定回归报告绑定 executable workspace
...
本清单没有授权 commit、push、merge、tag、release、deploy、用户级安装替换或业务项目迁移。

## 本次审计验证

- `openspec status --change delivery-integrity-hardening`：4/4 artifacts；
  `openspec validate delivery-integrity-hardening`：passed。
...
- 770-case targeted union：756 passed、14 skipped、0 failure/error；776-case
  discovery：762 passed、14 skipped、0 failure/error/expected-failure。
