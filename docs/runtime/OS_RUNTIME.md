...
Schema 23 records connector health in `connector_budgets` and adds `attempt_count`, `next_retry_at`, `connector_status`, and `blocked_reason` to `adapter_actions`. GitHub `gh api` calls and HTTP connectors share retry-aware handling for 429/529, common 5xx failures, GitHub rate-limit stderr/header signals, and `Retry-After`. Notion calls are throttled toward 2 req/s, Slack posting is throttled per channel, and Figma plan/tier headers are recorded as free-plan risk when present.
